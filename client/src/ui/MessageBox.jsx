...
const MessageBox = () => {
  ...
  return (
    <div className="bg-onyx  p-4 flex">
      <form className="flex-1 bg-charleston-Green  rounded-md">
        <input
          type="text"
          value={message}
          onChange={(e) => setMessage(e.target.value)} // Update the message based on user input
          placeholder="Message #General"
          className="w-full bg-discordDark-100 text-sonic-silver placeholder-sonic-silver focus:outline-none font-medium px-4 py-2 rounded-md"
        />
      </form>
    </div>
  );
